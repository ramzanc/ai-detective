...
def test_duplicate_ids_are_rejected() -> None:
    data = valid_case_data()
    duplicate = deepcopy(data["evidence"][0])
    data["evidence"].append(duplicate)

    with pytest.raises(ValidationError) as error:
        CaseManifest.model_validate(data)

    assert "duplicate id 'silver_watch'" in str(error.value)


def test_missing_character_reference_is_rejected() -> None:
    data = valid_case_data()
    data["timeline"][0]["actor_ids"] = ["unknown_character"]
    ...
    assert "timeline event 'maya_departure'" in message
    assert "unknown_character" in message


def test_missing_location_reference_is_rejected() -> None:
    data = valid_case_data()
    data["evidence"][0]["location_id"] = "unknown_location"
    ...
    assert "evidence 'silver_watch'" in message
    assert "unknown_location" in message


def test_missing_fact_reference_is_rejected() -> None:
    data = valid_case_data()
    data["evidence"][0]["reveals_fact_ids"] = ["unknown_fact"]
    ...
    assert "evidence 'silver_watch'" in message
    assert "unknown_fact" in message


def test_missing_evidence_reference_is_rejected() -> None:
    data = valid_case_data()
    data["hints"][0]["points_to_evidence_ids"] = ["missing_evidence"]
    ...
    assert "hint 'check_departure_time'" in message
    assert "missing_evidence" in message


def test_evidence_cannot_unlock_itself() -> None:
    data = valid_case_data()
    data["evidence"][0]["unlocks_evidence_ids"] = ["silver_watch"]

    with pytest.raises(ValidationError) as error:
        CaseManifest.model_validate(data)

    assert "evidence 'silver_watch' cannot unlock itself" in str(error.value)
